...
public class PrenotazioneController {
    ...
    /*@GetMapping("/camereDisponibili")
    public ResponseEntity<List<Camera>> findCamereByDate(
            @RequestParam("dataInizio") Date dataInizio,
            @RequestParam("dataFine") Date dataFine) {
        try {
            List<Camera> camereDisponibili = prenotazioneService.findCamereByDate(dataInizio,dataFine);
            if (camereDisponibili.isEmpty()) {
                return new ResponseEntity<>(null, HttpStatus.NO_CONTENT); // 204
            } else {
                return new ResponseEntity<>(camereDisponibili, HttpStatus.OK); // 200
            }
        } catch (Exception e) {
    ...
    public ResponseEntity<List<Prenotazione>> getAllPrenotazioni(){
        try {
            List<Prenotazione> listPrenotazione = prenotazioneService.findAll();
            if(listPrenotazione.isEmpty()){
                return new ResponseEntity<>(null, HttpStatus.NO_CONTENT); //204
            }
            else{
                return new ResponseEntity<>(listPrenotazione, HttpStatus.OK); //200
            }
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR); //500
        }
